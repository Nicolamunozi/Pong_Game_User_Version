...
def is_collision_player(player_paddle=player_1.paddle, ball=ball.ball):
    ...
    ball_y = ball.ycor()
    ball_x = ball.xcor()
    print(f"coordenadas. Player_x: {player_x}, player_y: {player_y}, ball_x: {ball_x}, ball_y: {ball_y}")    
    if int(ball_x - player_x) == int(STEP_ZISE):
        #Estamos en distancia de colision:
        print("distancia de colision")
        for idx in range(4):
            print(idx)
...
while game_is_on:
    ...
    while not ball.outside():
        ...
        # print(f"La coordenada de x más grande es: {max_x}/nLa coordenada de x más pequeña es {min_x}.")    
